...
namespace Level
{
    public class MapTile : MonoBehaviour
    {
        ...
        public Dictionary<Vector2Int, TileProperty>
            tiles = new(); //地图块的坐标和对应的Tile对象
        ...
        {
            var position =
                new Vector3(gridPosition.x * gridSize, 0, gridPosition.y * gridSize); //将Tilemap坐标转换为世界坐标用了X轴和Z轴
            return position;
        }

        public Vector2Int Position2TilemapPos(Vector3 position) //将世界坐标转换为Tilemap坐标
        {
            var gridX = Mathf.FloorToInt(position.x / gridSize);
            var gridY = Mathf.FloorToInt(position.y / gridSize);
        ...
        {
            if (tiles.ContainsKey(gridPosition)) return tiles[gridPosition];
